...
#if defined(NDEBUG)
# error "MP3Coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9e640581ee0f3fff36f89b8c2421bff41bfd168846409f202656a2e63da7b570");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MP3Coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MP3Coin Signed Message:\n";
...
{
    // MP3Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 0.5; // MP3Coin: twice per day
static const int64 nTargetSpacing = 30; // MP3Coin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MP3Coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MP3Coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MP3Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "08/16/2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x77ca6f14f19061c54bedaffc7ec65b2cfc5bee256c77a9501fc92b8576c8fee1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MP3Coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MP3CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MP3CoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MP3CoinMiner(CWallet *pwallet)
{
    printf("MP3CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mp3coin-miner");
    ...
    try { loop {
        ...
        printf("Running MP3CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MP3CoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MP3CoinMiner, pwallet));
}
